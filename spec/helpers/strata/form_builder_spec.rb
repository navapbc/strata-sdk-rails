...
RSpec.describe Strata::FormBuilder do
  ...
  describe '#money_field' do
    ...
    it 'defaults to inputmode decimal' do
      expect(result).to have_element(:input, inputmode: 'decimal')
    end

    it 'allows overridding inputmode' do
      result_with_inputmode = builder.money_field(:weekly_wage, inputmode: 'numeric')
      expect(result_with_inputmode).to have_element(:input, inputmode: 'numeric')
    end

    context 'with additional HTML options' do
      let(:result) { builder.money_field(:weekly_wage, data: { test: 'value' }, aria: { label: 'Amount' }, value: "50.5") }

      it 'passes through HTML options to the input' do
        expect(result).to have_element(:input, 'data-test': 'value', 'aria-label': 'Amount', value: '50.5')
      end
    end
  end
end
